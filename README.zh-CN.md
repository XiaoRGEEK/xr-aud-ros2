...
| XR-AUD-02 | Fusion | 在 Standard Audio 之外，通过已安装 Runtime 提供受 capability 约束的 Raw Array、DOA 和“唤醒词 + 方向”事件。 |
...
唤醒词配置格式可以公开，但当前用于内部验证的 KWS 模型权重许可尚未完成书面
确认，因此不能进入本仓库、DEB、镜像或公开下载。只有安装了许可明确、与 Runtime
兼容的 backend/model 包后，才能把唤醒事件作为公开产品能力交付。
